 ...
 */
DefaultSchemaStrategy.prototype.getTableName = function(tableName) {
...
DefaultSchemaStrategy.prototype.initializeSchema = function(db, callback) {
  ...
  const docsTable = this.getTableName('docs');
  const metaTable = this.getTableName('meta');
...
DefaultSchemaStrategy.prototype.initializeInventory = function(db, callback) {
  ...
  db.getFirstAsync(
      'SELECT data FROM ' + strategy.getTableName('meta') + ' WHERE id = ?',
      ['inventory'],
  ).then(function(row) {
    if (row) {
      ...
      return db.runAsync(
          'INSERT INTO ' + strategy.getTableName('meta') + ' (id, data) VALUES (?, ?)',
...
DefaultSchemaStrategy.prototype.deleteAllTables = function(db, callback) {
  ...
  promises.push(db.runAsync('DROP TABLE IF EXISTS ' + this.getTableName('meta')));
  promises.push(db.runAsync('DROP TABLE IF EXISTS ' + this.getTableName('docs')));
  promises.push(db.runAsync('DROP TABLE IF EXISTS ' + this.getTableName('inventory')));
