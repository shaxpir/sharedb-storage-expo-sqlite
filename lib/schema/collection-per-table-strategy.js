...
CollectionPerTableStrategy.prototype.initializeSchema = function(db, callback) {
  ...
  // Always create meta table with sharedb_ prefix
  ...
  console.log('[CollectionPerTableStrategy] Pre-configured collections:', collections);
  for (let i = 0; i < collections.length; i++) {
    const collection = collections[i];
    console.log('[CollectionPerTableStrategy] Creating table for collection:', collection);
    promises.push(this.createCollectionTable(db, collection));
  }

  Promise.all(promises).then(function() {
    console.log('[CollectionPerTableStrategy] All tables created successfully, schema initialized');
    strategy.debug && logger.info('CollectionPerTableStrategy: Schema initialized');
    callback && callback();
  }).catch(function(error) {
    console.error('[CollectionPerTableStrategy] Error during schema initialization:', error);
...
CollectionPerTableStrategy.prototype.createCollectionTable = function(db, collection) {
  ...
  return Promise.all(promises).then(function() {
    ...
    console.log('[CollectionPerTableStrategy] Successfully created table:', tableName, 'for collection:', collection);
  ...
  if (type === 'meta') {
    ...
    if (!collection || collection === 'docs') {
      ...
      db.getFirstAsync(
        ...
        const actualCollection = record.collection || collection;
