...
SqliteStorage.prototype.initialize = function(onReadyCallback) {
  ...
  console.log('[SqliteStorage] Starting initialization of database:', this.dbFileName);
  ...
  this.adapter.openDatabase(this.dbFileName, {}, this.dbFileDir, function(error, db) {
    ...
    console.log('[SqliteStorage] Database opened successfully, initializing schema...');
    ...
    storage.schemaStrategy.initializeSchema(dbWrapper, function(err) {
      if (err) {
        console.error('[SqliteStorage] Error initializing schema:', err);
        return onReadyCallback(err);
      }

      console.log('[SqliteStorage] Schema initialization complete, initializing inventory...');
      ...
      storage.schemaStrategy.initializeInventory(dbWrapper, function(err2, inventory) {
        if (err2) {
          console.error('[SqliteStorage] Error initializing inventory:', err2);
          return onReadyCallback(err2);
        }

        console.log('[SqliteStorage] Storage initialization COMPLETE - ready for use!');
        storage.ready = true;
