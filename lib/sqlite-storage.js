...
SqliteStorage.prototype.initialize = function(onReadyCallback) {
  ...
  this.adapter.openDatabase(this.dbFileName, {}, this.dbFileDir, function(error, db) {
    ...
    // Database opened, initialize schema
...
SqliteStorage.prototype._createDbWrapper = function() {
  ...
  return {
    ...
    runAsync: function(sql, params) {
      return {
        ...
        then: function(onSuccess, onError) {
          // Return a real promise for proper chaining
          return new Promise(function(resolve, reject) {
            adapter.run(sql, params || [], function(error, result) {
              if (error) {
                if (onError) {
                ...
                if (onSuccess) {
                  try {
                    const returnValue = onSuccess(result);
                    // Handle case where onSuccess returns a promise-like object
          ...
          return new Promise(function(resolve, reject) {
            adapter.get(sql, params || [], function(error, row) {
              if (error) {
                ...
                if (onSuccess) {
                  try {
                    const returnValue = onSuccess(row);
                    // Handle case where onSuccess returns a promise-like object
                    if (returnValue && typeof returnValue.then === 'function') {
                      returnValue.then(resolve, reject);
                    } else {
    ...
    transaction: function(transactionFn, callback) {
      adapter.transaction(transactionFn, callback);
    },
  };
};
...
SqliteStorage.prototype.readRecord = function(storeName, recordId, callback) {
  console.log('[SqliteStorage.readRecord] Called with storeName:', storeName, 'recordId:', recordId);
  this.ensureReady();
  ...
  console.log('[SqliteStorage.readRecord] type:', type, 'collection:', collection);
  ...
  try {
    ...
    this.schemaStrategy.readRecord(dbWrapper, type, collection, recordId, function(error, record) {
      if (callbackCalled) {
        ...
        console.error('[SqliteStorage.readRecord] Second call - error:', error, 'record:', record);
        if (error && error.stack) {
          console.error('[SqliteStorage.readRecord] Stack trace of duplicate call:', error.stack);
        }
        return;
      }
      callbackCalled = true;
      
      console.log('[SqliteStorage.readRecord] Callback called - error:', error, 'record:', record ? 'exists' : 'null');
      
      if (error) {
        console.error('[SqliteStorage.readRecord] Error from schemaStrategy:', error);
        console.error('Error reading record:', error);
        callback(error);
        return;
      }

      console.log('[SqliteStorage.readRecord] Record returned:', record ? 'exists' : 'null', 'has payload:', record ? !!record.payload : 'N/A');
      
      // Return just the payload for backward compatibility
      const payload = record ? record.payload : null;
      console.log('[SqliteStorage.readRecord] About to call callback with payload:', payload === null ? 'null' : 'object');
      callback(payload);
    });
  } catch (err) {
    console.error('[SqliteStorage.readRecord] CAUGHT ERROR in schemaStrategy.readRecord:', err);
    console.error('[SqliteStorage.readRecord] Stack trace:', err.stack);
...
SqliteStorage.prototype.readRecordsBulk = function(storeName, recordIds, callback) {
  console.log('[SqliteStorage.readRecordsBulk] Called with', recordIds.length, 'record IDs');
  this.ensureReady();
  ...
  if (this.schemaStrategy.readRecordsBulk) {
    console.log('[SqliteStorage.readRecordsBulk] Using schema strategy bulk read');
    this.schemaStrategy.readRecordsBulk(dbWrapper, type, collection, recordIds, callback);
  } else {
    console.log('[SqliteStorage.readRecordsBulk] Falling back to individual reads');
