...
ExpoSqliteAdapter.prototype.connect = function() {
  ...
  return new Promise(async function(resolve, reject) {
    try {
      // expo-sqlite automatically uses SQLite subdirectory when no directory is specified
      const dbOptions = { useNewConnection: true };
      adapter.db = adapter.SQLite.openDatabaseSync(adapter.fileName, dbOptions);

      adapter.debug && console.log('[ExpoSqliteAdapter] Connected to database: ' + adapter.fileName + ' in default SQLite directory');
