...
ExpoSqliteAdapter.prototype.connect = function() {
  ...
  return new Promise(async function(resolve, reject) {
    try {
      ...
      // If dirPath is the document directory, open directly there
      // Otherwise, treat it as a subdirectory path
      const fullPath = adapter.dirPath + adapter.fileName;
      const dbOptions = { useNewConnection: true };

      // expo-sqlite requires just the filename when opening from the default SQLite directory
      // But for custom paths, we need to handle it differently
      if (adapter.dirPath.endsWith('/SQLite/')) {
        // Opening from SQLite subdirectory - just use filename
        ...
        // Opening from custom directory - use full path
        adapter.db = adapter.SQLite.openDatabaseSync(fullPath, dbOptions);
        adapter.debug && console.log('[ExpoSqliteAdapter] Connected to database: ' + fullPath);
      }
