...
function AttachedExpoSqliteAdapter(primaryFileName, primaryDirPath, attachmentConfig, options) {
  ...
  const processedConfig = {
    attachments: (attachmentConfig.attachments || []).map(function(attachment) {
      ...
      // For expo-sqlite, both databases are in the SQLite directory
      // We just need the filename for the ATTACH statement
      // SQLite will look in the same directory as the primary database
      return {
